...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function() {
	...
	var A, c = Math.PI,
		...
		b = function(e, t, r, n, o, i, a, h, s) {
			if (e !== h || t !== s) {
				...
				var S = (i === a ? -1 : 1) * Q(x),
					...
					A = (-d - b) / n,
					R = T * T + P * P,
					O = (P < 0 ? -1 : 1) * Math.acos(T / Q(R)),
					L = (T * A - P * z < 0 ? -1 : 1) * Math.acos((T * z + P * A) / Q(R * (z * z + A * A)));
				...
				var I, F = Math.ceil(Math.abs(L) / (E / 4)),
					...
					D = g * -n,
					V = f * n;
				for (I = 0; I < F; I++) u = U(o = O + I * Y), d = q(o), T = U(o += Y), P = q(o), G.push(u - j * d, d + j * u, T + j * P, P - j * T, T, P);
				for (I = 0; I < G.length; I += 2) u = G[I], d = G[I + 1], G[I] = u * B + d * D + M, G[I + 1] = u * X + d * V + N;
		...
		j = function(e) {
			...
			for (t = 0; t < y; t++)
				...
				else if ('L' === i || 'Z' === i) 'Z' === i && (n = s, o = l, h.closed = !0), ('L' === i || 0.5 < Math.abs(m - n) || 0.5 < Math.abs(_ - o)) && (x(m, _, n, o), 'L' === i && (t += 2)), m = n, _ = o;
				else if ('A' === i) {
		...
		M = function(e, t) {
			var r, n, o, i, a, h, s, l, f, g, p, c, u, d, m = 0,
				...
				y = t / ((_ - 2) / 6);
			for (u = 2; u < _; u += 6)
				for (m += y; 0.999999 < m;) r = e[u - 2], n = e[u - 1], o = e[u], i = e[u + 1], a = e[u + 2], h = e[u + 3], s = e[u + 4], l = e[u + 5], f = r + (o - r) * (d = 1 / ((Math.floor(m) || 1) + 1)), f += ((p = o + (a - o) * d) - f) * d, p += (a + (s - a) * d - p) * d, g = n + (i - n) * d, g += ((c = i + (h - i) * d) - g) * d, c += (h + (l - h) * d - c) * d, e.splice(u, 4, r + (o - r) * d, n + (i - n) * d, f, g, f + (p - f) * d, g + (c - g) * d, p, c, a + (s - a) * d, h + (l - h) * d), u += 6, _ += 6, m--;
			return e;
		},
		B = function(e, t) {
			var r, n, o, i = '',
		...
		X = function(e) {
			...
			return e.centerX = (h + s) / 2, e.centerY = (l + f) / 2, e.left = s, e.width = h - s, e.top = f, e.height = l - f, e.size = (h - s) * (l - f);
		},
		P = function(e, t) {
		...
		V = function(e, t, r) {
			...
			if (r)
				for (n = e.slice(0), N(n), i = 6; i < a; i += 6)(o = _(n, t, i / 6, l, f)) < g && (g = o, p = -i);
			return p / 6;
		},
		W = function(e, t, r) {
		...
		Z = function(e, t, r, n, o, i) {
			var a, h, s, l, f = t.length,
				g = 0,
				p = Math.min(e.size || T(e), t[r].size || T(t[r])) * n,
				c = 999999999999,
				u = e.centerX + o,
				d = e.centerY + i;
			for (a = r; a < f && !((t[a].size || T(t[a])) < p); a++) h = t[a].centerX - u, s = t[a].centerY - d, (l = Q(h * h + s * s)) < c && (g = a, c = l);
		...
		k = function(e, t, r, n, o) {
			...
			if (u[0]) {
				...
				if (p)
					for (p < 0 && (p = -p), c[0].length > u[0].length && M(u[0], (c[0].length - u[0].length) / 6 | 0), y = u.length; d < p;) c[y].size || T(c[y]), s = (h = W(u, c[y].centerX, c[y].centerY))[0], l = h[1], u[y++] = [s, l, s, l, s, l, s, l], u.totalPoints += 8, d++;
				for (y = 0; y < e.length; y++) i = t[y], a = e[y], (p = i.length - a.length) < 0 ? M(i, -p / 6 | 0) : 0 < p && M(a, p / 6 | 0), v && !1 !== o && !a.reversed && N(a), (r = C[y] || 0 === C[y] ? C[y] : 'auto') && (a.closed || Math.abs(a[0] - a[a.length - 2]) < 0.5 && Math.abs(a[1] - a[a.length - 1]) < 0.5 ? 'auto' === r || 'log' === r ? (C[y] = r = V(a, i, !y || !1 === o), r < 0 && (v = !0, N(a), r = -r), D(a, 6 * r)) : 'reverse' !== r && (y && r < 0 && N(a), D(a, 6 * (r < 0 ? -r : r))) : !v && ('auto' === r && Math.abs(i[0] - a[0]) + Math.abs(i[1] - a[1]) + Math.abs(i[i.length - 2] - a[a.length - 2]) + Math.abs(i[i.length - 1] - a[a.length - 1]) > Math.abs(i[0] - a[a.length - 2]) + Math.abs(i[1] - a[a.length - 1]) + Math.abs(i[i.length - 2] - a[0]) + Math.abs(i[i.length - 1] - a[1]) || r % 2) ? (N(a), C[y] = -1, v = !0) : 'auto' === r ? C[y] = 0 : 'reverse' === r && (C[y] = -1), a.closed !== i.closed && (a.closed = i.closed = !1));
		...
		o = function(e, t) {
			...
			for (r = 0.5 * t / (0.5 * (o = e.length) - 1), n = 0; n < o - 2; n += 2) {
				if (l += r, h = parseFloat(e[n + 2]), s = parseFloat(e[n + 3]), 0.999999 < l)
					for (a = 1 / (Math.floor(l) + 1), i = 1; 0.999999 < l;) p += (f + (h - f) * a * i).toFixed(2) + ',' + (g + (s - g) * a * i).toFixed(2) + ' ', l--, i++;
				p += h + ',' + s + ' ', f = h, g = s;
			}
			return p;
		},
		...
		ee = function(e) {
			for (var t, r, n, o, i, a, h, s, l, f, g, p, c = e.length; - 1 < --c;) {
				for (g = (r = e[c]).isSmooth = r.isSmooth || [0, 0, 0, 0], p = r.smoothData = r.smoothData || [0, 0, 0, 0], g.length = 4, s = r.length - 2, h = 6; h < s; h += 6) n = r[h] - r[h - 2], o = r[h + 1] - r[h - 1], i = r[h + 2] - r[h], a = r[h + 3] - r[h + 1], l = C(o, n), f = C(a, i), (t = Math.abs(l - f) < 0.09) && (p[h - 2] = l, p[h + 2] = f, p[h - 1] = Q(n * n + o * o), p[h + 3] = Q(i * i + a * a)), g.push(t, t, 0, 0, t, t);
				r[s] === r[0] && r[s + 1] === r[1] && (n = r[0] - r[s - 2], o = r[1] - r[s - 1], i = r[2] - r[0], a = r[3] - r[1], l = C(o, n), f = C(a, i), Math.abs(l - f) < 0.09 && (p[s - 2] = l, p[2] = f, p[s - 1] = Q(n * n + o * o), p[3] = Q(i * i + a * a), g[s - 2] = g[s - 1] = !0));
			}
			return e;
		},
		te = function(e) {
		...
		ne = _gsScope._gsDefine.plugin({
			...
			init: function(e, t, r, n) {
				var o, i, a, h, s, l, f, g, p, c, u, d, m, _, y, C, v, x, S, w, b, M, N = window.getComputedStyle(e),
					T = N.fill + '',
					P = !('none' === T || '0' === (T.match(R) || [])[3] || 'evenodd' === N.fillRule),
				...
				if (l = t.shapeIndex || 0 === t.shapeIndex ? t.shapeIndex : 'auto', f = t.map || ne.defaultMap, this._prop = t.prop, this._render = t.render || ne.defaultRender, this._apply = 'updateTarget' in t ? t.updateTarget : ne.defaultUpdateTarget, this._rnd = Math.pow(10, isNaN(t.precision) ? 2 : +t.precision), this._tween = r, h) {
					if (this._target = e, v = 'object' == typeof t.precompile, c = this._prop ? e[this._prop] : e.getAttribute(i), this._prop || e.getAttributeNS(null, 'data-original') || e.setAttributeNS(null, 'data-original', c), 'd' === i || this._prop) {
						if (this._rawPath = c = j(v ? t.precompile[0] : c), u = j(v ? t.precompile[1] : h), !v && !k(c, u, l, f, P)) return !1;
						for ('log' !== t.precompile && !0 !== t.precompile || Y('precompile:["' + B(c) + '","' + B(u) + '"]'), (b = 'linear' !== (t.type || ne.defaultType)) && (c = ee(c), u = ee(u), c.size || X(c), u.size || X(u), w = te(z[0]), this._origin = c.origin = {
			...
			set: function(e) {
				...
				if (this._super.setRatio.call(this, e), 1 === e && this._apply)
					...
					for (r = e < 0.5 ? 2 * e * e : (4 - 2 * e) * e - 1; d;) c = (h = d.i) + (h === (o = u[d.j]).length - 4 ? 7 - o.length : 5), a = C(o[c] - o[h + 1], o[c - 1] - o[h]), g = q(a), p = U(a), l = o[h + 2], f = o[h + 3], i = d.l1s + r * d.l1c, o[h] = l - p * i, o[h + 1] = f - g * i, i = d.l2s + r * d.l2c, o[c - 1] = l + p * i, o[c] = f + g * i, d = d._next;
	...
	}, ne.pointsFilter = r, ne.subdivideRawBezier = ne.subdivideSegment = M, ne.rawPathToString = B, ne.defaultType = 'linear', ne.defaultUpdateTarget = !0, ne.defaultMap = 'size', ne.stringToRawPath = ne.pathDataToRawBezier = function(e) {
		return j($(e, !0));
...
function(e) {
	...
	'undefined' != typeof module && module.exports ? (require('../../../node_modules/gsap/src/uncompressed/TweenLite.js'), module.exports = t()) : 'function' == typeof define && define.amd && define(['TweenLite'], t);
